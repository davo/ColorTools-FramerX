...
const buildThresholdArray = () => Array.from(Array(colors.length).keys(), i => i / colors.length)
...
const itemsHeightTotal = (collection: string[], height: number) => (collection.length / 3) * height
...
export function ColorGrid(props) {
    ...
    useLayoutEffect(() => {
        ...
        setCardProps({ width: ColorCard.defaultProps._sizeOfMasterOnCanvas.width, height: ColorCard.defaultProps._sizeOfMasterOnCanvas.height })
    ...
    let wrapper: MotionStyle = {
        display: "grid",
        padding: 20,
        gridRowGap: 24, gridColumnGap: 24,
        gridAutoRows: `${cardProps.height}px`,
        gridTemplateColumns: `repeat(auto-fit, ${cardProps.width}px)`,
    }

    // ref={ref}>

    const current = (colorName) => React.useMemo(() => colorProps(colorName), [colorName])

    return (
        <>
            <Scroll width={width} height={height} background="red" contentHeight={contentHeight} overflow="visible">
                <Frame width={width} style={{ height: contentHeight, ...wrapper }} background="BlueViolet" overflow="visible">
                    ...
                    {colors.map((colorName, index) => {
                        ...
                        return (
                            <Frame width={cardProps.width} key={index} height={cardProps.height} style={{ position: "relative" }} background="none">
                                <ColorCard
